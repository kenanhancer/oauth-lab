...
class TestOAuthAuthorizationServerMetadata:
    async def test_returns_required_fields(self, http_client: AsyncClient) -> None:
        ...
        assert "urn:ietf:params:oauth:grant-type:token-exchange" in body["grant_types_supported"]
        # Exactly the six registered grants — derived from the GrantRegistry,
        # so a silently dropped registration would fail here.
        assert len(body["grant_types_supported"]) == 6
        ...
        # RFC 9207 §3 — the AS emits `iss` in authorization responses, so it
...
class TestOpenIDConnectMetadata:
    async def test_includes_oidc_specific_fields(self, http_client: AsyncClient) -> None:
        ...
        # Inherited from the OAuth document — all six grants and the
        # RFC 9207 §3 iss-parameter announcement.
        assert len(body["grant_types_supported"]) == 6
        assert body["authorization_response_iss_parameter_supported"] is True
